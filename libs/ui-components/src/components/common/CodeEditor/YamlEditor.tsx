...
const YamlEditor = <R extends FlightCtlYamlResource>({ apiObj, refetch, disabledEditReason }: YamlEditorProps<R>) => {
  ...
  const needsReload = resourceVersion !== apiObj.metadata.resourceVersion;
  ...
  const handleSave = async (updatedYaml: string) => {
    ...
    try {
      ...
      // The "test" operation ensures that if the resourceVersion has changed externally, the patch request will fail.
      patchList.unshift({
        op: 'test',
        path: '/metadata/resourceVersion',
        value: apiObj.metadata.resourceVersion,
      });

      const endpoint = getResourceEndpoint(updatedObj);
      ...
      setResourceVersion(newResourceVersion);
