...
const QuickStartGuide = () => {
  ...
  const panelClassName = [
    'quickStart-guide',
    isGuideMinimized ? 'quickStart-guide--minimized' : '',
    isDragging ? 'quickStart-guide--dragging' : '',
    panelStyle ? 'quickStart-guide--positioned' : '',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  return ReactDOM.createPortal(
    ...
    >
      {isGuideMinimized ? (
        <Card isCompact>
          <CardBody className="quickStart-guide__drag-handle" aria-label={t('Drag guide')}>
            <Flex alignItems={{ default: 'alignItemsCenter' }} spaceItems={{ default: 'spaceItemsSm' }}>
              <FlexItem flex={{ default: 'flex_1' }}>
                <Button variant="link" isInline className="quickStart-guide__restore" onClick={onExpandGuide}>
                  <Content component={ContentVariants.small} className="pf-v6-u-font-weight-bold">
                    {activePhase.title}
                  </Content>
                  <StepChipCounter className="quickStart-guide__minimized-step" />
                  <Content component={ContentVariants.small} className="quickStart-guide__minimized-title">
                    {t('Explore the console')}
                  </Content>
                </Button>
              </FlexItem>
        ...
        <Card>
          <CardHeader
            className="quickStart-guide__drag-handle"
