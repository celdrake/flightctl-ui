    ...
    <FormGroupWithHelperText label={t('Delta generation')} content={<DeltaGenerationHelpContent />}>
      <Stack hasGutter>
        ...
        {deltaGeneration.generateDelta && !orgHasDeltaRepository && (
          <StackItem>
            <Alert isInline variant="info" title={t('No delta repository in your organization')}>
              {t(
                'Your organization does not have an OCI registry configured to store deltas. Create one in Repositories or ask your administrator whether a deployment-wide repository is already configured.',
              )}{' '}
