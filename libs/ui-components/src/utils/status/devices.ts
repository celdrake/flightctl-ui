...
import { type StatusItem, type StatusLevel } from './common';
...
export const getDeviceSummaryStatusLevel = (status?: DeviceSummaryStatusType): StatusLevel =>
  (status && DEVICE_SUMMARY_STATUS_LEVELS[status]) || 'unknown';
...
export const getDeviceLifecycleStatusLevel = (status?: DeviceLifecycleStatusType): StatusLevel =>
  (status && DEVICE_LIFECYCLE_STATUS_LEVELS[status]) || 'unknown';
