...
export const getAppSummaryStatusLevel = (status?: AppSummaryStatus): StatusLevel =>
  (status && APP_SUMMARY_STATUS_LEVELS[status]) || 'unknown';

export const getAppStatusLevel = (status?: AppStatus): StatusLevel =>
  (status && APP_STATUS_LEVELS[status]) || 'unknown';
